...
#original_Topics = reverse_label_encoding(df_rts_temp['Topic_Code'], Topic_encoder)
...
def read_csv_and_return_variables():
    ...
    df_rts_temp = df_rts_temp_intermediate.groupby(['User_IP', 'Topic']).size().reset_index()
    ...
    Topic_counts = df_rts_temp['Topic'].value_counts()
    print(Topic_counts)
    ...
    df_rts_temp['Topic_Code'] = Topic_encoder.fit_transform(df_rts_temp['Topic']).astype(np.int32)

    ips_temp = user_ip_encoder.classes_
    comb_temp = Topic_encoder.classes_


    print(f"Nombre de User_IPs uniques: {len(ips_temp)}")
    print(f"Nombre de Topic uniques: {len(comb_temp)}")
    print(f"Lecture du CSV terminée en {time.time() - start_time:.2f} secondes.")
    
    return df_rts_temp, ips_temp, comb_temp,user_ip_encoder, Topic_encoder

def create_sparse_matrix(df_rts_temp):
    # Create user-Topic matrix
    num_users = len(df_rts_temp['User_IP_Code'].unique())
    num_Topics = len(df_rts_temp['Topic_Code'].unique())
    
    user_combo_matrix = sp.coo_matrix(
        (df_rts_temp['Connections'], (df_rts_temp['User_IP_Code'], df_rts_temp['Topic_Code'])),
...
def tfidf_weighted_cosine_similarity(user_combo_matrix):
    # Assuming 'user_combo_matrix' is a sparse matrix (users x Topics)
...
def main():
    df_rts, ips, combos, user_ip_encoder, Topic_encoder = read_csv_and_return_variables()
    ...
    threshold = 0.9# Adjust this based on the plot
   ...
   # print(f"Label Propagation Modularity: {modularity_label_prop}")
    ...
    df_leiden = pd.DataFrame({"user_id": list(range(len(leiden_communities.membership))),
                              "community_id": leiden_communities.membership})
    df_leiden.to_csv("leiden_communities_TOPIC.csv", index=False)
